...
public:
  ...
  void setTracks( std::vector<GarlicExtendedTrack* > & tracks ) {
    _conversions.clear();
    _inputTracks.clear();
    _inputTracks = tracks;
  }

  //  std::vector < std::pair < GarlicExtendedTrack* , GarlicExtendedTrack* > > getConversions() {
...
private:
  ...
  std::vector < GarlicExtendedTrack* > _inputTracks;

  //  std::vector < std::pair < GarlicExtendedTrack* , GarlicExtendedTrack* > > _conversions;

  std::vector < GarlicConversionInfo > _conversions;

  TVector2 getCircleCentre(const TrackState* trk);

  GarlicConversionInfo isConversion(GarlicExtendedTrack* trk1, GarlicExtendedTrack* trk2);
