...
void GarlicExtendedTrack::calculateTrk() {
  ...
  float bfield = GarlicGeometryParameters::Instance().Get_bField();
  ...
  // track @ CALO entrance
  //---------------------------
  // hmm, this trackstate is sometimes screwed up!
  // sometimes in wrong direction along track helix...
  ...
  if (_ttverbose) {
    ...
    cout << "track state @ calo: ref pt = ";
    for (int i=0; i<3; i++) cout << tst->getReferencePoint()[i] << " ";
    cout << endl;

    tst = _parentTrack->getTrackState( TrackState::AtLastHit );
    cout << "track state @ last hit: ref pt = ";
    for (int i=0; i<3; i++) cout << tst->getReferencePoint()[i] << " ";
    cout << endl;

    tst = _parentTrack->getTrackState( TrackState::AtIP );
    cout << "track state @ IP: ref pt = ";
    for (int i=0; i<3; i++) cout << tst->getReferencePoint()[i] << " ";
    cout << endl;

    cout << "first hit: pos = ";
    for (int i=0; i<3; i++) cout << hits[0]->getPosition()[i] << " ";
    cout << endl;

    cout << "last hit: pos = ";
    for (int i=0; i<3; i++) cout << hits[hits.size()-1]->getPosition()[i] << " ";
    cout << endl;
  }

  // now actually choose which track state
  tst = NULL;
  //  tst = _parentTrack->getTrackState( TrackState::AtCalorimeter ); // sometimes crap
  if ( !tst ) {
...
//  // maybe safer to get track state nearest to "outermost" hit
//  float mindist(9999999);
//  int ibest(-999);
//  for (size_t i=0; i<hits.size(); i++) {
//    float dist(0);
//    for (int j=0; j<3; j++) 
//      dist+=pow( hits[i]->getPosition()[j], 2);
//    dist = sqrt(dist);
//    if (dist<mindist) {
//      mindist=dist;
//      ibest=i;
...
//  }
//  if (ibest)
//    tst = _parentTrack->getClosestTrackState ( hits[ibest]->getPosition()[0], hits[ibest]->getPosition()[1], hits[ibest]->getPosition()[2] );
//  else
//    tst = _parentTrack->getTrackState( TrackState::AtIP );
  ...
  //  cout << "track helix ref point: " << tst->getReferencePoint()[0] << " " << tst->getReferencePoint()[1] << " " << tst->getReferencePoint()[2] << endl;
  //  cout << "innermost hit " << _parentTrack->getRadiusOfInnermostHit () << endl;
...
void GarlicExtendedTrack::calculateMC() {
  float position[3];
  float momentum[3];
  for (int i=0; i<3; i++) {
...
void GarlicExtendedTrack::calculate() {
  ...
  if (_ttverbose) {
    cout << "hello from GarlicExtendedTrack::calculate" << endl;
  ...
  if( fabs(_helixECAL->getMomentum()[0]) < 0.0000000001 && fabs(_helixECAL->getMomentum()[1]) < 0.0000000001 ) {
    cout << "GarlicExtendedTrack::calculate : Strange Track!" << endl;
  }

  float shortest_time=1.0e+10;

  float intersec[3]={0};
  for (int isec=0; isec<GarlicGeometryParameters::Instance().Get_symmetry()+2; isec++) {
    float time=-999;
    float xyz[3];
    ...
    switch (isec) {
    case 0:
      //      time = _helix->getPointInZ( GarlicGeometryParameters::Instance().Get_zOfEndcap(), refPoint, intersec);
      xyz[2]=GarlicGeometryParameters::Instance().Get_zOfEndcap() - _helixECALRefPoint[2]; // in Helix frame
      time = _helixECAL->getPointInZ( xyz[2], refPoint, intersec);
      // for (int i=0; i<3; i++) intersec[i]+=_helixECALRefPoint[i];
      break;
    case 1:
      xyz[2]=-GarlicGeometryParameters::Instance().Get_zOfEndcap() - _helixECALRefPoint[2];
    ...
    default:
      ...
      float dir[2];
      float point[2];
      float r = GarlicGeometryParameters::Instance().Get_rOfBarrel();
      for (int j=0; j<2; j++) {
	dir[j] = GarlicGeometryParameters::Instance().Get_barrelStaveDir()[stave][j];
	//	point[j] = dir[j]*r;
...
float GarlicExtendedTrack::getDistanceToPoint(const float* point, bool atIP) {
  ...
  if ( zone == CALHITZONE_UNKNOWN ) { 
    if ( fabs ( p.getPosition()[2] ) < GarlicGeometryParameters::Instance().Get_zOfEndcap() ) // treat as barrel
