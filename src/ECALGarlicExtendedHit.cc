...
float PointInCalo::getAbsThickness(int layer) {
  ...
  for (int il=0; il<layer; il++) {
    absThick+=ECALGarlicGeometryParameters::Instance().Get_absThicknessBarrelLayer()[il];
  }
  return absThick;
}
...
void PointInCalo::assignZone(bool check) {
  ...
  if (fabs(_pos[2])<=bar_end_boundary) {
    ...
    float rmin = ECALGarlicGeometryParameters::Instance().Get_positionBarrelLayer() [0];
    ...
    float rmax = ECALGarlicGeometryParameters::Instance().Get_rMaxOfBarrel();
	...
	// 
	// cout << ECALGarlicGeometryParameters::Instance().Get_nBarrelEcalLayers() << " " << rmax << endl;
	// 
	// for (int i=0; i<ECALGarlicGeometryParameters::Instance().Get_nBarrelEcalLayers(); i++) {
	// 	cout << i << " " << ECALGarlicGeometryParameters::Instance().Get_positionBarrelLayer() [i] << endl;
	// }
	// cout << "--------" << endl;
	assert(0);
      }
	...
	cout << endl << "WARNING, hit outside ENDCAP/RING zone..." << endl <<
	  ...
	  " barrel z = " << ECALGarlicGeometryParameters::Instance().Get_zOfBarrel() << endl <<
	  " endcap z = " << ECALGarlicGeometryParameters::Instance().Get_zOfEndcap() << endl <<
...
void ExtendedHit2::getRegion() {
  if (_layer>=0) return;
  _layer = (*ECALGarlicGeometryParameters::Instance().Get_defaultDecoder())(_hit)["K-1"];
  _stave = (*ECALGarlicGeometryParameters::Instance().Get_defaultDecoder())(_hit)["S-1"];
  _module = (*ECALGarlicGeometryParameters::Instance().Get_defaultDecoder())(_hit)["M"];
  return;
}
