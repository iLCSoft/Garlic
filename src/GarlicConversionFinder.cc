...
void GarlicConversionFinder::calculateConversions() {
  ...
  GarlicExtendedTrack* itrk[2]={0,0};
  for (size_t i1=0; i1<_inputTracks.size()-1; i1++) {
    ...
    for (size_t i2=i1+1; i2<_inputTracks.size(); i2++) {
      ...
      if ( (convInfo.selLoose || convInfo.selTight) ) {

	// if ( convInfo.selLoose || convInfo.selTight ) {
	// if ( numberOfElectrons==0 ) convInfo.selTight=false; // require at least one electron for tight conversions
...
//  }
  ...
  while (1) {
    ...
    std::vector < GarlicExtendedTrack* > multiplyUsedTracks;
    ...
    std::map <float, GarlicExtendedTrack*> orderedTracks;
    ...
    for ( std::map <float, GarlicExtendedTrack*>::reverse_iterator itt=orderedTracks.rbegin(); itt!=orderedTracks.rend(); itt++) {
...
GarlicConversionInfo GarlicConversionFinder::isConversion(GarlicExtendedTrack* trk1, GarlicExtendedTrack* trk2) {
...
GarlicConversionInfo GarlicConversionFinder::isConversion(Track* trk1, Track* trk2) {
  ...
  //cout << "conversion rejection? " << rejected << endl;
  //cout << info.dxy << " ( " << _max_dxy << " ), " << info.dz << " ( " << _max_dz << " ), " << info.angle << " ( " << _max_angle << " ) " << endl;
