...
void GarlicGeometryHelpers::getFrontFaceProj(const float* a, float* proj) {
  // project a point towards/away from the origin (IP) onto the ECAL front face
  float scale(0);

  float scaleBarrel = GarlicGeometryParameters::Instance().Get_rOfBarrel()/ sqrt(pow(a[0], 2) + pow(a[1], 2));
  float scaleEndcap = fabs(GarlicGeometryParameters::Instance().Get_zOfEndcap()/a[2]);

  if ( fabs ( scaleBarrel*a[2] ) < GarlicGeometryParameters::Instance().Get_zOfEndcap() )  // scaled position in barrel region
    scale = scaleBarrel;
  else
    scale = scaleEndcap;
  ...
  // // check my new fn
  // float pp[3];
  // GarlicGeometryHelpers::getFrontFaceProj(a, a, pp);
  // 
...
void GarlicGeometryHelpers::getLineCylinderIntersection( const float* aPoint, const float* aDir, float radius, float* intersec1, float* intersec2 ) {
...
void GarlicGeometryHelpers::getLineCircleIntersection( const float* aPoint, const float* aDir, float radius, float* intersec1, float* intersec2 ) {
...
void GarlicGeometryHelpers::getFrontFaceProj(const float* a, const float* dir, float* proj) {

  // cout << "point : ";for (int i=0; i<3; i++) cout << a[i] << " "; 
  // cout << " : dir "; for (int i=0; i<3; i++) cout << dir[i] << " "; 
  // cout << endl;
  // cout << sqrt( a[0]*a[0] + a[1]*a[1] ) << " - " << GarlicGeometryParameters::Instance().Get_rOfBarrel() << endl;
  ...
  bool intersectEndcap = rr <= GarlicGeometryParameters::Instance().Get_rOfBarrel();
   
  // now the barrel
  float barrelIntersection1[3];
  float barrelIntersection2[3];
  getLineCylinderIntersection( a, dir, GarlicGeometryParameters::Instance().Get_rOfBarrel(), barrelIntersection1, barrelIntersection2 );
  ...
  intersectBarrel1 = intersectBarrel1 && fabs ( barrelIntersection1[2] ) <= fabs(GarlicGeometryParameters::Instance().Get_zOfEndcap());
  intersectBarrel2 = intersectBarrel2 && fabs ( barrelIntersection2[2] ) <= fabs(GarlicGeometryParameters::Instance().Get_zOfEndcap());
...
double GarlicGeometryHelpers::Get2dProjDistance(const float* a, const float* b) {
  float aProj[3];
...
void GarlicGeometryHelpers::GetGeneralPointPlaneProjection(const float* point, const float* pointOnPlane, const float* planeNormal, const float* projDirection, float* projection) {
...
double GarlicGeometryHelpers::Get3dDistance(const float* a, const float* b) {
  return sqrt ( pow(a[0] - b[0],2) + pow(a[1] - b[1],2) + pow(a[2] - b[2],2) );
}

double GarlicGeometryHelpers::GetDistToLine(const float* a, const float* l1, const float* l2) {
...
void GarlicGeometryHelpers::cross(const float* a, const float* b, float* vec) {
...
float GarlicGeometryHelpers::dot(const float* a, const float* b) {
  return a[0]*b[0] + a[1]*b[1] + a[2]*b[2];
}

float GarlicGeometryHelpers::mag(const float* a) {
  return sqrt( pow(a[0], 2)+pow(a[1], 2)+pow(a[2], 2) );
}

void GarlicGeometryHelpers::diff(const float* a, const float* b, float* vec) {
  for (int i=0; i<3; i++) vec[i] = a[i]-b[i];
  return;
}

void GarlicGeometryHelpers::sum(const float* a, const float* b, float* vec) {
  for (int i=0; i<3; i++) vec[i] = a[i]+b[i];
  return;
}

float GarlicGeometryHelpers::cosangle(const float* a, const float* b) {
  return dot(a,b)/(mag(a)*mag(b));
}

float GarlicGeometryHelpers::angle(const float* a, const float* b) {
  return acos(cosangle(a, b));
}

float GarlicGeometryHelpers::phi(const float* a) {
  float ph = atan2(a[1],a[0]);
  if (ph<0) ph+=2*acos(-1);
  return ph;
}

float GarlicGeometryHelpers::costheta(const float* a) {
  return a[2]/mag(a);
}

float GarlicGeometryHelpers::theta(const float* a) {
  return acos(costheta(a));
}

float GarlicGeometryHelpers::getPseudoLayerRadius(int psLayer) {
  return GarlicGeometryParameters::Instance().Get_positionBarrelLayer() [psLayer];
}


float GarlicGeometryHelpers::getPseudoLayerDistNormal(int ps1, int ps2) {
  return GarlicGeometryParameters::Instance().Get_positionBarrelLayer()[ps2] - GarlicGeometryParameters::Instance().Get_positionBarrelLayer()[ps2];
}

int GarlicGeometryHelpers::getPseudoStave(const float* pos) {
  return int( (phi(pos) + acos(-1)/GarlicGeometryParameters::Instance().Get_symmetry()) / (2*acos(-1)/GarlicGeometryParameters::Instance().Get_symmetry()) );
}

float GarlicGeometryHelpers::getPseudoStavePhi(int istave) {
  return istave*(2*acos(-1)/GarlicGeometryParameters::Instance().Get_symmetry());
}

void GarlicGeometryHelpers::norm(const float* a, float* norma) {
  scale(a, mag(a), norma);
}

void GarlicGeometryHelpers::scale(const float* a, float factor, float* aprime) {
  for (int i=0; i<3; i++) aprime[i]=a[i]*factor;
}

float GarlicGeometryHelpers::det(const float* abcd) {
  /*
...
void GarlicGeometryHelpers::getLineBarrelPseudoLayerIntersection(const float* point1, const float* point2, int pseudolayer, float* intersection) {
  ...
  if (gotit) {
    ...
    cout << "GarlicGeometryHelpers::getLineBarrelPseudoLayerIntersection error: " << endl;
...
void GarlicGeometryHelpers::getPointsOnBarrelPseudoLayer(int pslayer, int pseudostave, float* point1, float* point2) {
...
void  GarlicGeometryHelpers::get2dLineIntersection(const float* p1, const float* p2, const float* p3, const float* p4, float* intersection) {
...
void GarlicGeometryHelpers::get3dLinePlaneIntersection(const float* planePoint, const float* planeNormal, 
